// Ported from runebender-xilem/src/model/entity_id.rs (Apache-2.0).
...
//! global atomic counter. IDs are used as keys in `Selection` sets and
//! for matching click targets to path elements during hit testing. They
//! are never reused within a session, so deleted points leave no
//! dangling references.
